...
import {
  ModifyFilterInstagramData,
  ModifyFilterTikTokData,
  ModifyInstagramData,
  ModifyTikTokData,
...
function HomePage() {
  ...
  useEffect(() => {
    setInstagramContents(
      ModifyInstagramData(instagramData?.getInstagramAccount, instagramLoading)
    );
  }, [instagramData]);
  useEffect(() => {
...
const FilteredContents = ({
  ...
  const [instagramContents, setInstagramContents] = useState<any[]>(
    ModifyFilterInstagramData(data?.instagrams)
  );

  const [tiktokContents, setTiktokContents] = useState<any[]>(
    ModifyFilterTikTokData(data?.tiktoks)
  );
  const deleteInstagramContents = (data: {
    posts: string[];
    reels: string[];
  ...
  return (
    <Box bg="#FAFAFA" px={5} pb={8} minH={"100vh"} width={"100%"}>
      ...
      >
        <Masonry gutter="10px">
          {[
            ...instagramContents?.map((account: any[], i: any) => {
